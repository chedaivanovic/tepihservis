<header>
    <div class="top-header d-flex justify-content-between">
        <div class="wrapper">
            <div class="d-flex justify-content-between w-100">
                <ul id="header-phones" class="d-flex">
                    <li><a href="tel:+381658700038">065/87-000-38</a></li>
                    <li><a href="tel:+0649399843">064/939-98-43</a></li>
                </ul>
                <!-- <ul class="social-links d-flex">
                    <li>
                        <a target="_blank" class="fb-icon" href="">
                            <i class="fab fa-facebook-f"></i>
                        </a>
                    </li>
                    <li>
                        <a target="_blank" class="insta-icon" href="">
                            <i class="fab fa-instagram"></i>
                        </a>
                    </li>
                </ul> -->
                <h1>S&J | Tepih Servis | Dubinsko pranje | Beograd</h1>
            </div>
        </div>
    </div>
    <div class="main-header">
        <div class="wrapper">
            <nav id="mainNav" class=" w-100 d-flex justify-content-between">
                <a href="#hero" class="main-logo-holder">
                    <img src="assets/img/S&J-logo.svg" alt="S&J Logo">
                </a>
                <button id="navbar-toggler" class="d-block d-sm-none">
                    <i class="fas fa-bars"></i>
                </button>
                <ul id="main-menu" class="d-sm-flex">
                    <li><a href="#about-us">O nama</a></li>
                    <!-- <li><a href="#gallery">Galerija</a></li> -->
                    <li><a href="#prices">Cenovnik</a></li>
                    <li><a href="#contact">Kontakt</a></li>
                </ul>

            </nav>
        </div>
    </div>
</header>